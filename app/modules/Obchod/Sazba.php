...
class Sazba extends Model 
{
	...
	public function show($id=0)
	{
		return dibi::query("SELECT	ts.id [tid], ts.nazev [typ], s.id [sid], ROUND(s.hodnota*100,2) [hodnota], 
									s.id_set_sazeb [idss], ts.zkratka, ts.poradi
								FROM typy_sazeb ts
	...
	public function find($id)
	{
		...
		return dibi::dataSource('SELECT ts.nazev, ROUND(s.hodnota*100,2) [hodnota], ts.zkratka
	...
	{
		return dibi::dataSource("SELECT ts.id, ts.zkratka, ts.nazev, ts.poradi, ss.hodnota, COALESCE(ss.id,0) [ids]
									FROM typy_sazeb ts
